...
pub fn ack(allocator: std.mem.Allocator, dst_ip: [4]u8, port: u16, duration: u32) void {
    const c = Caller.init(allocator, 5, duration, @constCast(&_ack), .{ .ip = dst_ip, .port = port });
    c.call();
}

fn _ack(dst_ip: [4]u8, port: u16) void {
    ...
    var tcp_h = TcpHeader.init(port, 0x10);
    tcp_h.ack = helpers.randomInt(u32, 1000, 50000);
